...
argparser.add_argument( "--pssm", type=str, default="", help='pssm file for the scaffold' )
...
def design(pose, tag, pssm_f, sfd_out=''):
    ...
    def get_all_atom_close_csts(pose, ligand_res_number, bb_only=False, sd=1.0):
        ...
        print(f'{len(cst_list)} cst generated')
    ...
    def read_pssm(pose, pssm_f):
        ...
        for i in range(1,pose.size()):
            labels = pdbinfo.get_reslabels(i)
            if 'RIFRES' in labels:
                if pose.residue(i).name() not in BAD_RIF:
                    RIFRES_list.append(i)
        # generate pssm based TO
        pssm_df = pd.read_csv(pssm_f,delim_whitespace=True,header=1)
        for i in range(1,pose.size()):
            if i not in RIFRES_list:
                res_aa_list[i] = ''
                res_pssm_df = pssm_df.loc[i]
                for aa in AAs:
                    if res_pssm_df[aa].values[0] >= PSSM_CUTOFF:
                        res_aa_list[i] += aa   
            else:
                res_aa_list[i] ='FILMVWYSTQNDERKH'
        # output TO
        TOs = ['<DesignRestrictions name="pssm_design_task">']
        for res in res_aa_list:
            TOs.append(f'<Action selector_logic="res{res}"    aas="{res_aa_list[res]}"/>')
        TOs.append('</DesignRestrictions>')
        TOs = '\n'.join(TOs)

        return TOs

    PER_RESIDUE_RES_SELECTOR = '\n'.join([f'<Index name="res{i+1}" resnums="{i+1}"/>' for i in range(pose.size()-1)])
    design_pssm_TO = read_pssm(pose,pssm_f)
    ...
    #this protocol moves the ligand too much during minimization, transfer this to
        <SCOREFXNS>
            <ScoreFunction name="sfxn_design" weights="beta">
              ...
              <Reweight scoretype="atom_pair_constraint" weight="0.3"/> # original 0.3, change to 1 to improve cst. weight suggest value: 1 sd = 0.5?
              <Reweight scoretype="dihedral_constraint" weight="0.1"/>
              <Reweight scoretype="angle_constraint" weight="0.1"/>
              <Reweight scoretype="aa_composition" weight="1.0" />
            </ScoreFunction>
            <ScoreFunction name="sfxn_relax" weights="beta">
                <Reweight scoretype="atom_pair_constraint" weight="0.3"/> # original 0.3, change to 1 to improve cst. weight suggest value: 1 sd = 0.5?
            </ScoreFunction>
        ...
        <AddResidueLabel name="label_interface" residue_selector="interface" label="interface" />
            ...
            <FastDesign name="fastDesign_stage1" scorefxn="sfxn_design" repeats="1" task_operations="precompute_ig,pssm_design_task,ex1_ex2aro,ic,limitchi2,noCys,restrict_packing_rifres_res,upweight_interface,restrict_to_packing_not_interface,restrict_target2repacking" batch="false" ramp_down_constraints="false" cartesian="False" bondangle="false" bondlength="false" min_type="dfpmin_armijo_nonmonotone" relaxscript="MonomerDesign2019"/> 
            
            <FastDesign name="fastDesign_stage2" scorefxn="sfxn_design" repeats="3" task_operations="precompute_ig,pssm_design_task,ex1_ex2aro,ic,limitchi2,noCys,upweight_interface,restrict_to_packing_not_interface,restrict_target2repacking" batch="false" ramp_down_constraints="false" cartesian="False" bondangle="false" bondlength="false" min_type="dfpmin_armijo_nonmonotone" relaxscript="MonomerDesign2019"/> 
        ...
        <MOVERS>
            ...
            <DumpPdb name="after_fd1" fname="after_fd1.pdb" tag_time="1" scorefxn="sfxn" />
            <DumpPdb name="after_fd2" fname="after_fd2.pdb" tag_time="1" scorefxn="sfxn" />
            <DumpPdb name="after_relax" fname="after_relax.pdb" tag_time="1" scorefxn="sfxn" />
            <DumpPdb name="after_rmsd" fname="after_rmsd.pdb" tag_time="1" scorefxn="sfxn" />
        </MOVERS>
        ...
        <PROTOCOLS>
          ...
          Add filter="mismatch_probability"/> remove for apptainer
    ...
    cst_list = get_all_atom_close_csts(pose, ligand_res_number)
    for cst in cst_list:
        pose.add_constraint(cst)
    print('added cst')
...
def main( tag, silent_structure, sfd_in, sfd_out ):
    ...
    design( pose, tag, pssm_f, sfd_out )
